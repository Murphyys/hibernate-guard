...
- **⚠️ สาเหตุอันดับ 1 ของ "ทำไมไม่ hibernate" นับจากนี้ — flag ค้างจาก session อื่น:** ice เปิด Claude Code หลาย session พร้อมกัน (ตอน arm มี 5 flag สดพร้อมกัน) ตราบใดที่มี flag สักอันอยู่ = busy = ไม่หลับ ซึ่ง**ถูกต้องตาม design** แต่หน้าตาเหมือนพังเป๊ะ. session ที่ตายโดยไม่ยิง `Stop` จะทิ้ง flag ค้างได้ถึง 6 ชม. (`staleFlagHours`) เช็คก่อนไล่ debug อย่างอื่นเสมอ:
  ```powershell
  Get-ChildItem "$env:TEMP\HibernateGuard-busy" -Filter *.flag | Select-Object Name,LastWriteTime
  ```
- **⚠️ fail-open ที่ยังไม่ได้แก้ — busy dir หายแล้วถือว่าไม่ busy:** `watcher.ps1:70` (`Test-Busy`) ถ้า `$busyDir` ไม่มีอยู่จะ `return $false` = ไม่ busy → hibernate ต่อ. ตอน busy dir อยู่ใน install dir ความเสี่ยงต่ำ แต่ตอนนี้ย้ายมา `%TEMP%` ซึ่ง **Storage Sense / Disk Cleanup ลบได้** ถ้าโดนลบตอนมี session ทำงานอยู่ = หลับทับงาน. ช่องแคบ (hook สร้าง dir คืนทุกครั้งที่ `-Action busy`) แต่ควรแก้ให้ "dir หาย" ถือว่า busy หรือ log warning แทนที่จะเงียบๆ ปล่อยผ่าน — รอ ice ตัดสินใจ
